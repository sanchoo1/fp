...
if __name__ == '__main__':
    ...
    while True:
        ...
        if kinect.has_new_depth_frame():
            ...
            # 定义红点的位置
            red_point = [800, 500]  # 在图像中心画一个红点
            radius = 5  # 红点的半径
            color = (0, 0, 255)  # 红色 (BGR格式)
            thickness = -1  # 填充圆形

            # 在图像上画红点
            cv2.circle(colorImage, red_point, radius, color, thickness)

            depth_frame = kinect.get_last_depth_frame()
            depth_img = depth_frame.reshape((kinect.depth_frame_desc.Height, kinect.depth_frame_desc.Width))
            depth_img = cv2.flip(depth_img, 1)

            dpoint = color_point_2_depth_point(kinect, _DepthSpacePoint, kinect._depth_frame_data, red_point)
            print(dpoint)
            

            cv2.circle(depth_img, dpoint, radius, color, thickness)
            depth_img = util.drawDepth(depth_img)

            cv2.imshow('Test Depth View', depth_img)
            cv2.imshow('Test Color View', cv2.resize(colorImage, (int(1920/2), int(1080/2))))
            ...
            color_2_depth_space(kinect, _ColorSpacePoint, kinect._depth_frame_data, show=True, return_aligned_image=True)
